<x-office-layout title="{{$data->id ? 'Update' : 'Create'}} Credit Note">
    <!--begin::Toolbar-->
        ...
        <div id="kt_app_toolbar_container" class="app-container container-fluid d-flex flex-stack">
            ...
            <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                ...
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                    ...
                    <!--end::Item-->
                    <!--begin::Item-->
                    <li class="breadcrumb-item text-muted">Credit Note</li>
                    <!--end::Item-->
                    <!--begin::Item-->
                    <li class="breadcrumb-item">
                        <span class="bullet bg-gray-400 w-5px h-2px"></span>
    ...
    <div id="kt_app_content" class="app-content flex-column-fluid py-3 py-lg-6 animation-class delay3">
        <div id="kt_app_content_container" class="app-container container-fluid">
            <div class="card">
                <form class="form w-100" novalidate="novalidate" id="form_input" data-redirect-url="{{route('office.finance.credit.index')}}" action="{{$data->id ? route('office.finance.credit.update',$data->id) : route('office.finance.credit.store')}}">
                    <div class="card-header border-0 pt-6">
                        <div class="card-title">
                            <div class="d-flex align-items-center position-relative my-1">
                                <h1>Form {{$data->id ? 'Update' : 'Create'}} Credit Note</h1>
                            </div>
                        </div>
                        <div class="card-toolbar">
                            <div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
                                <a id="back_form_button" href="{{route('office.finance.credit.index')}}" class="btn btn-primary btn-sm btn-hover-scale menu-link">
                        ...
                        <div class="form-group row">
                            <div class="col-4 mb-3">
                                <div class="form-group">
                                    <label class="form-label text-gray-700 fw-bold" for="invoice_id">Select Invoice</label>
                                    <select name="invoice_id" id="invoice_id" class="form-select">
                                        @foreach ($invoices as $invoice)
                                            <option value="{{ $invoice->id }}" {{ $invoice->id === $data->invoice_id || $invoice->id === old('invoice_id') ? 'selected' : '' }}>{{ $invoice->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                    ...
                    <div class="card-footer">
                        ...
                        @if($data->id)
                        <button type="button" onclick="handle_confirm('Are you sure want to delete this Proposal ?', 'Yes, i`m sure', 'No, i`m not','DELETE','{{route('office.finance.credit.destroy',$data->id)}}');" class="btn btn-sm btn-danger">
                            Delete
                        </button>
                        @endif
                    </div>
