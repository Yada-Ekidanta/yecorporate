<x-office-layout title="{{ $data->id ? 'Update' : 'Create' }} Meeting Attendees">
    <!--begin::Toolbar-->
        ...
        <div id="kt_app_toolbar_container" class="app-container container-fluid d-flex flex-stack">
            ...
            <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                ...
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                    ...
                    <!--end::Item-->
                    <li class="breadcrumb-item text-muted">Meeting Attendees</li>
                    <!--end::Item-->
                    <!--begin::Item-->
                    <li class="breadcrumb-item">
                        <span class="bullet bg-gray-400 w-5px h-2px"></span>
    ...
    <div id="kt_app_content" class="app-content flex-column-fluid py-3 py-lg-6 animation-class delay3">
        <div id="kt_app_content_container" class="app-container container-fluid">
            <div class="card">
                <form class="form w-100" novalidate="" id="form_input" data-redirect-url="{{ route('office.crm.meeting-attendees.index') }}"
                    action="{{ $data->id ? route('office.crm.meeting-attendees.update', $data->id) : route('office.crm.meeting-attendees.store') }}">
                    <div class="card-header border-0 pt-6">
                        <div class="card-title">
                            <div class="d-flex align-items-center position-relative my-1">
                                <h1>Form {{ $data->id ? 'Update' : 'Create' }} Meeting Attendess</h1>
                            </div>
                        </div>
                        <div class="card-toolbar">
                            <div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
                                <a id="back_form_button" href="{{ route('office.crm.meeting-attendees.index') }}" class="btn btn-primary btn-sm btn-hover-scale menu-link">
                            ...
                            <div class="col-md-6 mb-3">
                                <div class="form-group">
                                    <select name="meeting_id" id="meeting_id" class="form-select">
                                        <option disabled selected>Select Meeting</option>
                                        @foreach ($meeting as $item)
                                            <option value="{{ $item->id }}" {{ $data->meeting_id === $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="form-group">
                                    <select name="attendees_employee" id="attendees_employee" class="form-select">
                                        ...
                                        @foreach ($employee as $item)
                                            <option value="{{ $item->id }}" {{ $data->attendees_employee === $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="form-group">
                                    <select name="attendees_contact" id="attendees_contact" class="form-select">
                                        ...
                                        @foreach ($contact as $item)
                                            <option value="{{ $item->id }}" {{ $data->attendees_contact === $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="form-group">
                                    <select name="attendees_lead" id="attendees_lead" class="form-select">
                                        ...
                                        @foreach ($lead as $item)
                                            <option value="{{ $item->id }}" {{ $data->attendees_lead === $item->id ? 'selected' : '' }}>{{ $item->title }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                    ...
                    <div class="card-footer">
                        ...
                        @if ($data->id)
                            <button type="button" onclick="handle_confirm('Are you sure want to delete this Opportunity ?', 'Yes, i`m sure', 'No, i`m not','DELETE','{{ route('office.crm.meeting-attendees.destroy', $data->id) }}');"
