<x-office-layout title="{{$data->id ? 'Update' : 'Create'}} Opportunity">
    ...
    <div id="kt_app_content" class="app-content flex-column-fluid py-3 py-lg-6 animation-class delay3">
        <div id="kt_app_content_container" class="app-container container-fluid">
            <div class="card">
                <form class="form w-100" novalidate="novalidate" id="form_input" data-redirect-url="{{route('office.crm.opportunity.index')}}" action="{{$data->id ? route('office.crm.opportunity.update',$data->id) : route('office.crm.opportunity.store')}}">
                    ...
                    <div class="col-4 mb-3">
                        <div class="form-floating">
                            <select name="client_contact_id" id="client_contact_id" class="form-control">
                                <option selected>Select Client Contact</option>
                                @foreach ($client as $item)
                                    <option value="{{ $item->id }}"
                                        {{ $item->client_contact_id === $data->id ? 'selected' : '' }}>
                                        {{ $item->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="col-4 mb-3">
                        <div class="form-floating">
                            <select name="opportunities_stage_id" id="opportunitie_stage_id" class="form-control">
                                <option selected>Select Opportunities Stage</option>
                                @foreach ($opportunityStage as $item)
                                    <option value="{{ $item->id }}"
                                        {{ $item->opportunities_stage === $data->id ? 'selected' : '' }}>
                                        {{ $item->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="col-4 mb-3">
                        <div class="form-floating">
                            <input type="number" class="form-control" id="probability" name="probability"
                                placeholder="Enter Probability" value="{{ $data->probability }}" />
                    ...
                    <div class="col-4 mb-3">
                        <div class="form-floating">
                            <input type="date" class="form-control" id="close_date" name="close_date"
                                placeholder="Enter Close Date" value="{{ $data->close_date }}" />
                        ...
                        <div class="form-floating">
                            <select name="lead_source_id" id="lead_source_id" class="form-control">
                                ...
                                @foreach ($leadSource as $item)
                                    <option value="{{ $item->id }}"
                                        {{ $item->lead_source === $data->id ? 'selected' : '' }}>
                                        {{ $item->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="card-footer">
                        ...
                        @if($data->id)
                        <button type="button" onclick="handle_confirm('Are you sure want to delete this Opportunity ?', 'Yes, i`m sure', 'No, i`m not','DELETE','{{route('office.crm.opportunity.destroy',$data->id)}}');" class="btn btn-sm btn-danger">
                            Delete
                        </button>
                        @endif
                    </div>
