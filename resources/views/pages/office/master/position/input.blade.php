<x-office-layout title="{{$data->id ? 'Update' : 'Create'}} Position">
    ...
    <div id="kt_app_content" class="app-content flex-column-fluid py-3 py-lg-6 animation-class delay3">
        <div id="kt_app_content_container" class="app-container container-fluid">
            <div class="card">
                <form class="form w-100" novalidate="novalidate" id="form_input" data-redirect-url="{{route('office.hrm.master.department.show',$department->id)}}" action="{{$data->id ? route('office.hrm.master.position.update',$data->id) : route('office.hrm.master.position.store')}}">
                    <div class="card-header border-0 pt-6">
                        <div class="card-title">
                            <div class="d-flex align-items-center position-relative my-1">
                                <h1>Form {{$data->id ? 'Update' : 'Create'}} Position</h1>
                            </div>
                        </div>
                        <div class="card-toolbar">
                            <div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
                                <a id="back_form_button" href="{{route('office.hrm.master.department.show',$department->id)}}" class="btn btn-primary btn-sm btn-hover-scale menu-link">
                    ...
                    <div class="card-body transition-fade">
                        <div class="form-group row">
                            ...
                            <div class="col-4 mb-3">
                                <div class="form-floating">
                                    <input name="department_id" hidden class="form-control" cols="30" value="{{$department->id}}" rows="10">
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="card-footer">
                        ...
                        @if($data->id)
                        <button type="button" onclick="handle_confirm_custom('Are you sure want to delete this position ?', 'Yes, i`m sure', 'No, i`m not','DELETE','{{route('office.hrm.master.position.destroy',$data->id)}}', '{{ route('office.hrm.master.position.index') }}');" class="btn btn-sm btn-danger">
                            Delete
                        </button>
                        @endif
                    </div>
