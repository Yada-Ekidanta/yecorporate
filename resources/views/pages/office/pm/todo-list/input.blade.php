            ...
            <div class="card">
                <form class="form w-100" novalidate="novalidate" id="form_input" data-redirect-url="{{$data->id ? route('office.pm.task.show', request()->query('id')) : route('office.pm.task.show', $task->id)}}?id={{ request()->query('id') }}" action="{{$data->id ? route('office.pm.todo-list.update',$data->id) : route('office.pm.todo-list.store', $task->id)}}">
                    ...
                    <div class="card-footer">
                        ...
                        @if($data->id)
                        <button type="button" onclick="handle_confirm('Are you sure want to delete this bank ?', 'Yes, i`m sure', 'No, i`m not','DELETE','{{route('office.pm.todo-list.destroy',$data->id)}}');" class="btn btn-sm btn-danger">
                            Delete
                        </button>
                        @endif
                    </div>
