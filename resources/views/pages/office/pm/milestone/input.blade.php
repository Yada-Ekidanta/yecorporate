            ...
            <div class="card">
                <form class="form w-100" novalidate="novalidate" id="form_input" data-redirect-url="{{$data->id ? route('office.pm.project.show', request()->query('id')) : route('office.pm.project.show', $project->id) }}" action="{{$data->id ? route('office.pm.milestone.update',$data->id) : route('office.pm.milestone.store', $project->id)}}">
                    ...
                    <div class="card-footer">
                        ...
                        @if($data->id)
                            <button type="button" onclick="handle_confirm_custom('Are you sure want to delete this bank ?', 'Yes, i`m sure', 'No, i`m not','DELETE','{{route('office.pm.milestone.destroy',$data->id)}}', '{{ route('office.pm.project.show', request()->query('id')) }}');" class="btn btn-sm btn-danger">
                                Delete
                            </button>
                        @endif
                    </div>
