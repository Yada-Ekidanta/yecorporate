<x-office-layout title="{{$data->id ? 'Update' : 'Create'}} Company Industry">
    <!--begin::Toolbar-->
            ...
            <div class="card">
                <form class="form w-100" novalidate="novalidate" id="form_input" data-redirect-url="{{route('office.master.company-industry.index')}}" action="{{$data->id ? route('office.master.company-industry.update',$data->id) : route('office.master.company-industry.store')}}">
                    <div class="card-header border-0 pt-6">
                        <div class="card-title">
                            <div class="d-flex align-items-center position-relative my-1">
                                <h1>Form {{$data->id ? 'Update' : 'Create'}} Company Industry</h1>
                            </div>
                        </div>
                        ...
                        <div class="form-group row">
                            <div class="col-4 mb-3">
                                <div class="form-floating">
                                    <input type="text" class="form-control" id="name" name="name" placeholder="Company Industry" value="{{$data->name}}"/>
                    ...
                    <div class="card-footer">
                        ...
                        @if($data->id)
                        <button type="button" onclick="handle_confirm_custom('Are you sure want to delete this Company Industry ?', 'Yes, i`m sure', 'No, i`m not','DELETE','{{route('office.master.company-industry.destroy',$data->id)}}','{{route('office.master.company-industry.index')}}');" class="btn btn-sm btn-danger">
                            Delete
                        </button>
                        @endif
                    </div>
